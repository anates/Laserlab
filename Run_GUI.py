...
class Run_CM110_Thread(QRunnable):
	...
	def __init__(self,*argv):
		...
		self.avg_pts = argv[5]
		self.file_txt = argv[6]
		self.file_hdf5 = argv[7]
		self.timestr = argv[8]
		self.analogRef = argv[9]
		self.inst_list = argv[10]
		self.start_st = argv[11]
		self.stop_st = argv[12]
		self.step_st = argv[13]
	...
	def scan(self):
		...
		stepper_scanlist = numpy.arange(self.start_st,self.stop_st+self.step_st,self.step_st)
		
		for step_pos in stepper_scanlist:
			if self.end_flag:
				return
			if self.inst_list.get("SMC100"):
			...
			for start,stop,step,dwell in zip(self.start_,self.stop,self.step,self.dwell_time):
				...
				if self.inst_list.get("CM110"):
					self.inst_list.get("CM110").set_goto(start)
				
				# define STEPSIZE
				if self.inst_list.get("CM110"):
					self.inst_list.get("CM110").set_stepsize(step)
				
				for set_position in wv_scanlist:
